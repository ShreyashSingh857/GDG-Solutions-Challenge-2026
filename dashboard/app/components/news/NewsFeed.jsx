        ...
        {!newsAlerts.length ? (
          <div className="h-full flex flex-col items-center justify-center text-center py-20 px-6 space-y-6">
            <div className="relative">
              ...
              <div className="w-20 h-20 rounded-[32px] bg-[var(--bg-elevated)] flex items-center justify-center relative border border-[var(--border-subtle)] glass-panel">
                <ShieldCheck className="w-10 h-10 text-[var(--text-muted)] opacity-40" />
              </div>
            </div>
            <div className="space-y-2">
              <h3 className="text-sm font-bold uppercase tracking-[0.2em] text-[var(--text-primary)]">Scanning Horizons</h3>
            ...
            <button
              onClick={handleRefresh}
              disabled={isRefreshing}
              className="px-6 py-2 rounded-xl bg-[var(--bg-elevated)] border border-[var(--border-subtle)] text-[10px] font-bold uppercase tracking-widest hover:text-[var(--accent-cyan)] hover:border-[var(--accent-cyan)]/40 transition-all disabled:opacity-50"
            >
              {isRefreshing ? 'Polling Sources...' : 'Refresh Intelligence'}
            </button>
          </div>
        ) : (
          newsAlerts.map((alert) => {
            ...
            return (
              <article
                key={alert.id}
                className={`group relative flex flex-col glass-panel !bg-[var(--glass-bg)]/40 transition-all duration-500 hover:!bg-[var(--glass-bg-elevated)]/60 p-5 ${isHighSeverity ? '!border-[var(--accent-red)]/30 shadow-[0_4px_24px_rgba(239,68,68,0.1)]' : ''}`}
              >
                {isHighSeverity && (
                ...
                <div className="mt-4 pt-4 border-t border-[var(--border-subtle)] flex items-center justify-between">
                  <div className="flex gap-2">
                    {alert.affectedCorridors?.slice(0, 2).map((c) => (
                      <span key={c} className="text-[9px] font-bold uppercase tracking-wider text-[var(--text-muted)] bg-[var(--bg-elevated)]/40 px-2 py-1 rounded-md border border-[var(--border-subtle)]">
                        {c}
                      </span>
                    ))}
                  </div>
                  <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-[var(--text-muted)] group-hover:text-[var(--text-primary)] transition-colors">
