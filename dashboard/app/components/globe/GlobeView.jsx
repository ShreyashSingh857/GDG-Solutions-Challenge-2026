...
export default function GlobeView() {
  ...
  return <div className="relative w-full h-full bg-[#020617]"><GlobeControls onFilterChange={setF} />{t && <div className="fixed z-20 bg-black/80 border border-white/10 rounded-lg p-3 text-xs text-white" style={{ left: t.x + 12, top: t.y - 40 }}><p className="font-medium">{t.label}</p><p className="text-white/60">{t.carrier}</p><p className="text-white/60">${(t.cargoValueUSD / 1e6).toFixed(1)}M cargo</p></div>}<div className="absolute bottom-4 right-4 z-10 bg-black/50 backdrop-blur-md border border-white/10 rounded-xl px-4 py-2 flex gap-4 text-xs"><span className="text-green-400">{s.filter((x) => x.status === 'active').length} active</span><span className="text-red-400">{s.filter((x) => x.status === 'delayed').length} delayed</span><span className="text-blue-400">{s.filter((x) => x.status === 'rerouted').length} rerouted</span></div><Globe ref={globeRef} backgroundColor="#020617" globeImageUrl="//unpkg.com/three-globe/example/img/earth-night.jpg" atmosphereColor="#1e3a5f" atmosphereAltitude={0.15} pointsData={pts} pointLat="lat" pointLng="lng" pointColor="color" pointRadius="size" pointAltitude={0.01} onPointHover={(p, c) => setT(p ? { ...p, x: c?.clientX || 200, y: c?.clientY || 200 } : null)} arcsData={arcs} arcStartLat="startLat" arcStartLng="startLng" arcEndLat="endLat" arcEndLng="endLng" arcColor="color" arcAltitude={0.15} arcStroke={(x) => (x.status === 'delayed' ? 2.5 : x.status === 'rerouted' ? 3 : 1.5)} arcDashLength={0.5} arcDashGap={0.15} arcDashAnimateTime={(arc) => arc.status === 'delayed' ? 800 : arc.status === 'active' ? 2000 : 0} polygonsData={polys} polygonCapColor={() => 'rgba(239,68,68,.15)'} polygonSideColor={() => 'rgba(239,68,68,.05)'} polygonStrokeColor={() => 'rgba(239,68,68,.6)'} polygonAltitude={0.005} /></div>;
}
