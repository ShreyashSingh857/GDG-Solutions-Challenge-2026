  ...
  return (
    <nav className="h-12 shrink-0 flex items-center justify-between px-4 bg-black/60 backdrop-blur-md border-b border-white/5 z-40">
      ...
      <div className="flex items-center gap-1">
        ...
        <Link
          href="/replay"
          className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${pathname.startsWith('/replay') ? 'bg-white/10 text-white' : 'text-white/50 hover:text-white hover:bg-white/5'}`}
        >
          Replay
        </Link>
        <Link
        ...
        <Link
          href="/health"
          className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${pathname.startsWith('/health') ? 'bg-white/10 text-white' : 'text-white/50 hover:text-white hover:bg-white/5'}`}
        >
          System Health
        </Link>
      </div>
